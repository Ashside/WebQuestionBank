...
func QueryQuestions(db *gorm.DB, tableName, selectFields string, conditions []string, args ...interface{}) ([]QuestionSummary, error) {
	...
	if tableName == "choice_questions" {
		...
		selectFields = fmt.Sprintf("%s.id as id, 'simpleAnswer' as type, %s.content as question, %s.difficulty, %s.subject, %s.author_id as author", tableName, tableName, tableName, tableName, tableName)
	}
