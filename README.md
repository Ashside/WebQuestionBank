...
在`src/backend/config/config.yaml`中，实时配置你的数据库和服务器地址，注意不要随意更改字段名
如有必要更改，请在`src/backend/api/Database.go`中，对应修改`conf`结构体成员变量。

具体的说明可以自行查看`gorm`框架相关使用文档。
...
即可启动后端服务器，监听地址在后端配置文件中修改。

服务器默认监听8081端口，端口号在代码中修改。
